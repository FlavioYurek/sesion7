...
document.addEventListener("DOMContentLoaded", function () {
  var capturarIdBtn = document.getElementById("capturaridBtn");
  capturarIdBtn.addEventListener("click", function () {
    chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
      setTimeout(function () {
        console.log (chrome.tabs.sendMessage(tabs[0].id, { action: "capturarIdBtn" }));
      }, 500); // Espera 500 milisegundos antes de enviar el mensaje
    });
  });
});
